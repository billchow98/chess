...
void MoveGenerator::update_boardinfo() {
    bi.king_bb = board.bb(piece::King, board.turn);
    bi.king_sq = board.king_sq(board.turn);
}

Bitboard MoveGenerator::get_to_mask(Type type) {
    switch (type) {
    case Evasions:
        if (bb::popcnt(board.checkers) >= 2) {
            return ~board.color_bb[board.turn];
        } else {
            return board.evasion_mask();
        }
    case Captures:
        return board.color_bb[!board.turn];
    case Quiets:
...
void MoveGenerator::add(Square from, Square to, Piece promotion) {
    moves.push_back(move::init(from, to, promotion));
}

Bitboard MoveGenerator::rank_8() {
    return board.turn == color::White ? bb::RANK_8 : bb::RANK_1;
}

Bitboard MoveGenerator::single_pushes() {
    return board.single_pushes(board.bb(piece::Pawn, board.turn));
}
...
void MoveGenerator::generate_single_pushes(Bitboard to_mask) {
    ...
    while (tos != bb::Empty) {
        ...
        add(square::sub(to, dir::N, board.turn), to);
    }
}

Bitboard MoveGenerator::double_pushes() {
    return board.double_pushes(board.bb(piece::Pawn, board.turn));
}
...
void MoveGenerator::generate_quiet_queen_promotion(Type type,
    ...
    while (tos != bb::Empty) {
        auto to = bb::next_sq(tos);
        add(square::sub(to, dir::N, board.turn), to, piece::Queen);
    }
}
...
void MoveGenerator::generate_quiet_underpromotions(Type type,
    ...
    while (tos != bb::Empty) {
        ...
        for (Piece pc = piece::Rook; pc >= piece::Knight; pc--) {
            add(square::sub(to, dir::N, board.turn), to, pc);
        }
    }
}
...
Bitboard MoveGenerator::pawn_captures(Direction d) {
    auto pawns = board.bb(piece::Pawn, board.turn);
    // Do not & with color_bb[!turn] here. to_mask may contain ep square
    return bb::shift(pawns, d, board.turn);
}
...
void MoveGenerator::generate_normal_pawn_captures(Direction d,
    ...
    while (tos != bb::Empty) {
        auto to = bb::next_sq(tos);
        add(square::sub(to, d, board.turn), to);
    }
}
...
void MoveGenerator::generate_promotion_captures(Direction d, Bitboard to_mask) {
    ...
    while (tos != bb::Empty) {
        ...
        for (Piece pc = piece::Queen; pc >= piece::Knight; pc--) {
            add(square::sub(to, d, board.turn), to, pc);
        }
    }
}
...
void MoveGenerator::generate_piece(Piece pc, Bitboard froms, Bitboard to_mask) {
    ...
    while (froms != bb::Empty) {
        ...
        auto tos = lookup::attacks(pc, from, board.all()) & to_mask;
        while (tos != bb::Empty) {
            auto to = bb::next_sq(tos);
            add(from, to);
        }
...
void MoveGenerator::generate(Type type) {
    update_boardinfo();
    moves.clear();
    moves.reserve(cfg::MOVE_VEC_RESERVE_CAP);
    auto to_mask = get_to_mask(type);
    if (bb::popcnt(board.checkers) >= 2) {
        generate_piece(piece::King, bi.king_bb, to_mask);
        return;
    }
    generate_pawn(type, to_mask);
    for (Piece pc = piece::Knight; pc <= piece::Queen; pc++) {
        auto pc_bb = board.bb(pc, board.turn);
        generate_piece(pc, pc_bb, to_mask);
    }
    if (type == Evasions) {
        to_mask = ~board.color_bb[board.turn];
    }
    generate_piece(piece::King, bi.king_bb, to_mask);
...
void filter_legal(MoveGenerator &gen) {
    std::erase_if(gen.moves,
                  [&gen](Move move) { return !gen.board.is_legal(move); });
}
...
u64 perft(Board &board, i32 depth, bool is_root) {
    if (depth == 0) {
        return 1;
    }
    u64 ttl = 0;
...
u64 perft_pseudo_legal(Board &board, i32 depth, bool is_root) {
    if (depth == 0) {
        return 1;
    }
    u64 ttl = 0;
